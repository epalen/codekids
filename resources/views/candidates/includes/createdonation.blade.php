            ...
            <div class="row">                
                <div class="col-md-8 col-sm-12">
                    <form class="form-horizontal" method="POST" id="payment-form" role="form" action="{!! URL::route('addmoney.paypal') !!}" >
                        <input type="hidden" id="id" name="id" value="{{ $candidates->id }}">
                        <input type="hidden" id="name" name="name" value="{{ $candidates->name }} {{ $candidates->last_name }}">
                        <input type="hidden" id="type" name="type" value="candidato">
                        {{ csrf_field() }}
                        <article class="single-event">
                        <div class="event-thumb">
                            <a href="#">
                                <img src="{{ URL::asset('uploads/candidates/' . $candidates->avatar) }}" alt="" width="380" height="340">
                            </a>
                            <p class="event-post-date">
                                {{ Carbon::parse($candidates->created_at)->format('d') }}
                                <span>
                                    {{ Carbon::parse($candidates->created_at)->format('M') }}
                                </span>
                            </p>
                        </div>
                        <div class="event-content">
                            <h5 class="single-event-title">
                                {{ $candidates->name }} {{ $candidates->last_name }}
                            </h5>
                            
                                <div class="donate-information">
                                @foreach($balances as $balance)
                                    <div class="single-donate-information">
                                        <p><span>{{ number_format((($balance->total/$candidates->goal) * 100)) }} %</span>Alcanzado</p>
                                    </div>
                                    <div class="single-donate-information">
                                        <p><span>US$ {{ $balance->total }}</span>Sobre meta de ${{ $candidates->goal }}</p>
                                    </div> 
                                @endforeach                              
                                </div>
                                <div class="donate-information">
                            ...
                            <div class="donate-information">
                                <div class="single-donate-information">
                                    <p><span>Gustos</span>{{ $candidates->likes }}</p>
                                </div>
                                <div class="single-donate-information">
                                    <p><span>Talentos</span>{{ $candidates->talent }}</p>
                                </div>
                                <div class="single-donate-information">
                                    <p><span>Deseos</span>{{ $candidates->wishes }}</p>
                                </div>
                                <div class="single-donate-information">
                                    <p><span>Edad</span>
                            ...
                            <div class="donate-information">
                                <div class="single-donate-information">
                                    <p><span>Grupo</span>{{ $candidates->group->name }}</p>
                                </div>
                                <div class="single-donate-information">
                                    <p><span>Curso</span>{{ $candidates->course->name }}</p>
                                </div>
                            </div>
                            <div class="donate-information">
                                <p align="justify"><span>Descripción</span>{{ $candidates->description }}</p>
                            </div>
                            <br>
